...
#         self.use_model_control = False    # Flag to switch between keyboard and model control
...
#             else:
#                 print("Model or scaler files not found. Keyboard control will be used.")
#         except Exception as e:
#             print(f"Error loading model: {e}")
#             print("Falling back to keyboard control.")

#     def get_model_control(self, state_features):
#         """Get control actions from the trained model"""
#         if self.model is None or self.scaler is None:
#             return None
        
#         try:
#             # Scale the features
#             scaled_features = self.scaler.transform([state_features])
            
#             # Get model prediction
#             predictions = self.model.predict(scaled_features)[0]
            
#             # Extract actions
#             accel, brake, steer = predictions
...
#             # Print model predictions and current state
#             print("\n=== Model Predictions ===")
#             print(f"Current Speed: {current_speed:.2f}, Current RPM: {current_rpm:.2f}, Current Gear: {current_gear}")
#             print(f"Raw predictions: accel={predictions[0]:.3f}, brake={predictions[1]:.3f}, steer={predictions[2]:.3f}")
#             print(f"After constraints: accel={accel:.3f}, brake={brake:.3f}, steer={steer:.3f}, gear={gear}")
#             print("=======================\n")
            
#             return brake, accel, steer, gear
#         except Exception as e:
...
#         # Toggle between keyboard and model control with 'M' key
#         keys = pygame.key.get_pressed()
#         if keys[pygame.K_m]:
#             self.use_model_control = not self.use_model_control
#             print(f"Switched to {'model' if self.use_model_control else 'keyboard'} control")
#             time.sleep(0.5)  # Prevent multiple toggles
        
#         if self.use_model_control:
#             # Prepare features for the model (matching train_torcs_model.py features)
#             state_features = [
#                 self.state.getAngle(),
#                 self.state.getRpm(),
...
#             ] + list(self.state.getTrack())
            
#             # Get model predictions
#             model_actions = self.get_model_control(state_features)
            
#             if model_actions is not None:
#                 brake, accel, steer, gear = model_actions
#                 self.control.setBrake(brake)
#                 self.control.setAccel(accel)
#                 self.control.setSteer(steer)
#                 self.control.setGear(gear)
#         else:
#             # Use existing keyboard control
#             if self.use_sensor_steering:
#                 self.steer()
#             self.controls()
        
#         # Update previous values
#         self.prev_speedX = self.state.getSpeedX()
#         self.prev_steer = self.control.getSteer()
#         self.prev_opponents = self.state.getOpponents()
#         self.prev_closest_distance = min(self.state.getOpponents())
